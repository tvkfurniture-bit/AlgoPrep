...
def mode_standard_threshold(gray: np.ndarray, thresh: int, clahe: bool) -> np.ndarray:
    ...
    For: Digital art, logos, AI-generated images on white backgrounds.
    """
    if clahe:
...
def mode_photo_dither(gray: np.ndarray, gamma: float, contrast: float) -> np.ndarray:
    """
    Mode 3 — Photo / Grayscale Dither
    Adaptive Floyd-Steinberg dithering optimized for wood-grain engraving.
    """
    ...
    # Step 3: Contrast stretch (Safe division check)
    ...
    # Step 5: Dithering call with backward compatibility safety
...
def mode_edge_engrave(gray: np.ndarray, lo: int, hi: int, thickness: int) -> np.ndarray:
    ...
    Edge isolation via Canny extraction with dynamic pixel dilation.
    """
...
# Define safety fallbacks to prevent scope resolution errors in Streamlit
invert_out = False
...
with st.spinner("Processing image…"):
    ...
    # Convert PIL Image to gray array
    ...
    # Apply final morphological clean-up pass
    if denoise_on:
...
with info_col:
    ...
    st.markdown(f"""<div class="info" style="margin:0;">
      <b>AlgoOS Import Instructions:</b><br>
      {algoos_note[mode]}<br>
